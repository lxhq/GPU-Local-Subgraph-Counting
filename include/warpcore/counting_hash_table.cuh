...
public:
    ...
    CountingHashTable(
        ...
        base_table_(min_capacity, prealloc_table_ptr, prealloc_status_ptr, prealloc_tmp_ptr),
        max_count_(std::numeric_limits<value_type>::max()),
        is_copy_(false)
    {
        init(base_table_.seed());
    }
    ...
    /*! \brief inserts a key into the hash table with a specific value
     * \param[in] key_in key to insert into the hash table
     * \param[in] value_in value to added into the hash table
     * \param[in] group cooperative group
     * \param[in] probing_length maximum number of probing attempts
     * \return status (per thread)
     */
        ...
        index_type probing_length = defaults::probing_length()) noexcept
    {
        ...
        if(group.thread_rank() == 0 && value_ptr != nullptr)
        {
            ...
            if(*value_ptr < max_count_)
            {
                const value_type old = atomicAdd(value_ptr, value_in);
