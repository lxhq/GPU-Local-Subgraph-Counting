...
public:
    ...
    /*! \brief pre-allocated memory constructor
     * \param[in] min_capacity minimum number of slots in the hash table
     * \param[in] prealloc_table_ptr pointer to pre-allocated table memory
     * \param[in] prealloc_status_ptr pointer to pre-allocated status memory
     */
    HOSTQUALIFIER INLINEQUALIFIER
    SingleValueHashTable(
        const index_type min_capacity,
        void* prealloc_table_ptr,
        void* prealloc_status_ptr,
        void* prealloc_tmp_ptr) noexcept :
        status_(static_cast<status_type*>(prealloc_status_ptr)),
        table_(detail::get_valid_capacity(min_capacity, cg_size()), prealloc_table_ptr),
        temp_(TempMemoryBytes / sizeof(index_type), static_cast<index_type*>(prealloc_tmp_ptr)),
        seed_(defaults::seed<key_type>()),
        is_initialized_(false),
        is_copy_(false),
        is_pre_alloc_(true)
    {
    ...
    SingleValueHashTable(SingleValueHashTable&& o) noexcept :
        ...
        is_pre_alloc_(std::move(o.is_pre_alloc_))
    {
        o.is_copy_ = true;
    }
...
private:
    ...
    bool is_pre_alloc_; //< indicates if table uses pre-allocated memory
