...
public:
    ...
    /*! \brief pre-allocate constructor
     * \param[in] capacity buffer capacity
     */
    HOSTQUALIFIER INLINEQUALIFIER
    explicit CyclicStore(const index_type capacity, base_type* pre_allocate) noexcept :
        store_(nullptr),
        capacity_(capacity),
        current_(nullptr),
        status_(status_type::not_initialized()),
...
private:
    ...
    bool is_copy_;
    bool is_pre_alloc_; //< indicates if this object uses pre-allocated memory
...
public:
     ...
     * \param[in] capacity number of key/value slots
     * \param[in] prealloc_ptr pointer to pre-allocated memory
     */
    HOSTQUALIFIER INLINEQUALIFIER
    AoSStore(const index_type capacity, void* prealloc_ptr) noexcept :
        status_(status_type::none()),
        capacity_(capacity),
        pairs_(static_cast<pair_t*>(prealloc_ptr)),
        is_copy_(false),
        is_pre_alloc_(true)
    {}
